...
async function updateRecentPosts() {
    ...
    // ONLY UPDATE IF WE ACTUALLY FOUND POSTS
    if (cjPosts.length > 0 || elainePosts.length > 0) {
        console.log('✅ UPDATING DISPLAY WITH FOUND POSTS');
        ...
        console.log('⚠️ NO POSTS FOUND, CHECK PATHS');
    }
}

// RUN AFTER PAGE LOADS WITH DELAY
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', () => setTimeout(updateRecentPosts, 500));
} else {
    setTimeout(updateRecentPosts, 500);
}
